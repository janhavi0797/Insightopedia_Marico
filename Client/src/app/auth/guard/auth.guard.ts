...
export const authGuard: CanActivateFn = (route, state) => {
  ...
  // Role-based access control for userRole = 3
if (userRole === 3) {
  const allowedUrls = ['/portal/project-analysis'];
  if (!allowedUrls.includes(state.url)) {
    router.navigate(['/portal/project-analysis']);
    return false;
  }
}
// Role-based access control for userRole = 3
if (userRole === 2) {
  const allowedUrls = ['/portal/dashboard', '/portal/all-files','/portal/create-project','/portal/project-analysis'];
  if (!allowedUrls.includes(state.url)) {
