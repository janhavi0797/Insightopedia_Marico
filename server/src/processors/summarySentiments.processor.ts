...
import { AudioEntity } from "src/project/entity";
...
export class SummarySentimentsProcessor {
  ...
  async handleTranscriptionJob(job: Job) {
    const { updatedTextArray, combinedTranslation, audioId, fileName } = job.data;
    ...
    try {
      ...
      // Save to Cosmos DB
      const val = await this.audioUtils.saveTranscriptionDocument(transcriptionDocument);
      await job.log(JSON.stringify(val));
      await job.log(`Saved transcription document for ${audioId} without vectorId`);

      const embeddingJob = { transcriptionDocument, combinedTranslation, audioId, fileName };
